...
def main():
    ...
    train_dir = r"/Users/yourname/Desktop/OPTICAL/train"
    test_dir = r"/Users/yourname/Desktop/OPTICAL/test"

    # Build datasets separately
    train_videos = build_dataset(train_dir)
    test_videos = build_dataset(test_dir)

    if len(train_videos) == 0 or len(test_videos) == 0:
        print("❌ Missing training or testing videos. Exiting.")
        return

    # Extract features separately
    X_train, y_train = extract_dataset_features(train_videos)
    X_test, y_test = extract_dataset_features(test_videos)
    ...
    # Train SVM
    print("\n🚀 Training SVM on provided training set...")
    param_grid = {
        'C': [0.1, 1, 10, 100],
        'gamma': ['scale', 0.01, 0.001, 0.0001],
        'kernel': ['rbf']
    ...
    # Save model + scaler
    output = {'model': best_model, 'scaler': scaler}
